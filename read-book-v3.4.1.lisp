...
;; 開発ゴール
;; utils.lispから依存ライブラリの廃止[2020-06-12 11:42:31]
;; 具体的にはquicklispに依存しているものを取り除く。
;;
...
;; (load "~/quicklisp/dists/quicklisp/software/cl-rainbow/cl-rainbow.asd"
;; 			:external-format :utf-8)					; => T
;; (load "~/quicklisp/dists/quicklisp/software/cl-ppcre-2.0.11/cl-ppcre.asd"
;; 			:external-format :utf-8)					; => T
;; (load "~/quicklisp/dists/quicklisp/software/dexador-20170630-git/dexador.asd"
;; 			:external-format :utf-8)					; => T
;; (or (ql:quickload :trivial-shell :silent t)
;;     (require 'trivial-shell)						; =>
;;     (asdf:load-system :trivial-shell)
;;     (load "~/quicklisp/dists/quicklisp/software/trivial-shell-20170630-git/trivial-shell.asd"
;; 					:external-format :utf-8))			; => (:TRIVIAL-SHELL)
;; (ql:quickload :trivial-shell)						; => (:TRIVIAL-SHELL)
;; (load "~/.roswell/lisp/quicklisp/dists/quicklisp/software/trivial-shell-20180228-git/trivial-shell.asd"
;; 					:external-format :utf-8)			; => T
;; (ql:quickload :trivial-shell)						; => (:TRIVIAL-SHELL)
;; (asdf:initialize-source-registry
;;  '(:source-registry
;; 	 (:tree (:home "howm/junk/"))
;; 	 (:directory "~/quicklisp/dists/quicklisp/software/")
;; 	 :INHERIT-CONFIGURATION))							; =>

;; asdf:*source-registry-parameter*				; => (:SOURCE-REGISTRY (:TREE (:HOME "howm/junk/"))
;;  (:DIRECTORY "~/quicklisp/dists/quicklisp/software/") :INHERIT-CONFIGURATION)
;; quicklispのhomeを調べる時は以下を実行する。[2017-06-13 14:57:02]
;;ql:*quicklisp-home*					; => #P"/home/hiro/quicklisp/"

(ql:quickload '(:trivial-shell :cl-ppcre :dexador :cl-mecab :cl-rainbow :cl-interpol) :silent t) ; => (:TRIVIAL-SHELL :CL-PPCRE :DEXADOR :CL-MECAB :CL-RAINBOW :CL-INTERPOL)
;; (or (ql:quickload :trivial-shell :silent t)
;;     (require 'trivial-shell)
;;     (asdf:load-system :trivial-shell)
;;     (load "~/quicklisp/dists/quicklisp/software/trivial-shell-20170630-git/trivial-shell.asd"
...
(named-readtables:in-readtable :interpol-syntax) ;=> (("COMMON-LISP-USER" . #<NAMED-READTABLE :INTERPOL-SYNTAX {1003D990A3}>)
;; ("SB-VM" . #<READTABLE {1006473A13}>)
;; ("SB-FORMAT" . #<READTABLE {1006473A13}>)
;; ("SB-POSIX" . #<READTABLE {1006473A13}>)
;; ("SB-PROFILE" . #<READTABLE {1006473A13}>)
;; ("SB-ALIEN" . #<READTABLE {1006473A13}>) ("SB-C" . #<READTABLE {1006473A13}>)
;; ("SB-X86-64-ASM" . #<READTABLE {1006473A13}>)
;; ("SB-IMPL" . #<READTABLE {1006473A13}>)
;; ("SB-REGALLOC" . #<READTABLE {1006473A13}>)
;; ("SB-SYS" . #<READTABLE {1006473A13}>)
;; ("SB-THREAD" . #<READTABLE {1006473A13}>)
;; ("SB-BIGNUM" . #<READTABLE {1006473A13}>)
;; ("SB-GRAY" . #<READTABLE {1006473A13}>)
;; ("SB-LOOP" . #<READTABLE {1006473A13}>)
;; ("SB-KERNEL" . #<READTABLE {1006473A13}>)
;; ("SB-EXT" . #<READTABLE {1006473A13}>)
;; ("SB-ALIEN-INTERNALS" . #<READTABLE {1006473A13}>)
;; ("SB-DISASSEM" . #<READTABLE {1006473A13}>)
;; ("SB-BSD-SOCKETS-INTERNAL" . #<READTABLE {1006473A13}>)
;; ("SB-MOP" . #<READTABLE {1006473A13}>)
;; ("SB-PRETTY" . #<READTABLE {1006473A13}>)
;; ("SB-WALKER" . #<READTABLE {1006473A13}>)
;; ("SB-INT" . #<READTABLE {1006473A13}>) ("SB-UNIX" . #<READTABLE {1006473A13}>)
;; ("SB-ASSEM" . #<READTABLE {1006473A13}>)
;; ("SB-EVAL" . #<READTABLE {1006473A13}>)
;; ("SB-CLTL2" . #<READTABLE {1006473A13}>)
;; ("SB-UNICODE" . #<READTABLE {1006473A13}>)
;; ("SB-DEBUG" . #<READTABLE {1006473A13}>)
;; ("SB-PCL" . #<READTABLE {1006473A13}>)
;; ("SB-BSD-SOCKETS" . #<READTABLE {1006473A13}>)
;; ("SB-SEQUENCE" . #<READTABLE {1006473A13}>)
;; ("SB-FASL" . #<READTABLE {1006473A13}>)
;; ("SB-INTROSPECT" . #<READTABLE {1006473A13}>)
;; ("SB-DI" . #<READTABLE {1006473A13}>))

;; 色を付けたい時t,付けたくない時nil
;;(setf cl-rainbow:*enabled* t)		 ; => T
(setf cl-rainbow:*enabled* nil)		 ; => T
;; [重要]色をつけない設定にしてから$ read-book.lisp > outfile
...
;;(web-scraping "test")                   ;=> ("(能力などをためす)試験、検査、(ものの)検査、実験、ためすもの、試金石、試験の手段、試験、分析、鑑識")
