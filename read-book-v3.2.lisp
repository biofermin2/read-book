...
;; 開発ゴール
;; 今回の開発ではしおり機能を追加する。
;; 前回読んでた途中箇所から開始するかどうか訊いて、
;; yesの場合、途中から、noの場合最初から読み込みを
;; 開始する。
...
(load-edic "~/howm/junk/edic.db")				; => #<HASH-TABLE :TEST EQUALP :COUNT 3483 {E0A3EF9}>
(load-edic-cnt "~/howm/junk/edic-cnt.db") ; => #<HASH-TABLE :TEST EQUALP :COUNT 3483 {E108119}>
(load-last-char "~/howm/junk/last-char.db") ; => #<HASH-TABLE :TEST EQUAL :COUNT 2 {E122071}>
...
;; 	(with-open-file (in filchar; 		(with-standard-charyntax
...
(defun read-book (file)
	...
	(let* ((lst nil)
				...
				(ofile (make-pathname
								...
								:type "out")))
		(bookmark file)
		(with-open-file (in ofile :direction :input)
 			(do ((char (read-char in nil 'eof)
				...
				(when (or (and (not (equal lst "")) ;lstが空ではなく、かつ
					...
					(prin1 lst)
					(setf (gethash (concatenate 'string file (stamp-write-date file)) *last-char*) char-cnt)
					(save-last-char)
